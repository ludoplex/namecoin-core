...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_log(self):
        ...
        conf_path = default_datadir / "namecoin.conf"
    ...
    def test_ignored_conf(self):
        ...
        # error.
        self.start_node(0, [f'-conf={node.datadir}/namecoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        node = self.nodes[0]
        (default_datadir / "namecoin.conf").write_text(f"datadir={node.datadir}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir}" contains a "namecoin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/namecoin.conf" from data directory "{default_datadir}" '
    ...
    def run_test(self):
        ...
        none_existent_conf_file = os.path.join(default_data_dir, "none_existent_namecoin.conf")
        self.nodes[0].assert_start_raises_init_error(
